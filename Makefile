postgres:
	docker run --name db-service -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=postgres -d postgres:12-alpine

createdb:
	docker exec -it db-service createdb --username=root --owner=root simple_bank

dropdb:
	docker exec -it db-service dropdb simple_bank
